...
def _get_or_create_folder(session, parent_id: str, name: str) -> str:
    safe = name.replace("'", "\\'")
    q = (
        f"name='{safe}' and '{parent_id}' in parents "
        f"and mimeType='application/vnd.google-apps.folder' and trashed=false"
    )
    r = session.get(f"{_DRIVE}/files", params={"q": q, "fields": "files(id)"})
    r.raise_for_status()
...
def upload_cv_to_drive(
    ...
    try:
        ...
        contractor_folder_id = _get_or_create_folder(session, root_folder_id, contractor_name)
        safe_name = _safe_filename(filename)
        content_type = mime_type or _mime_from_filename(filename)

        logger.info("Uploading '%s' (%d bytes, %s) to folder %s",
                    safe_name, len(file_bytes), content_type, contractor_folder_id)

        # multipart/related upload: metadata + binary in one request
        boundary = "drive_cv_boundary"
        ...
        resp = session.post(
            f"{_UPLOAD}/files?uploadType=multipart&fields=id",
            headers={"Content-Type": f"multipart/related; boundary={boundary}"},
            data=body,
        )
        if not resp.ok:
            logger.error("Drive upload HTTP %s: %s", resp.status_code, resp.text)
        ...
        if not perm_resp.ok:
            logger.warning("Could not set Drive permissions for %s: %s", file_id, perm_resp.text)
